		<ul class="megadropdown primary-nav">
				<li class="main-nav-item">
					<a href="/drupal/node/127" class="withDD main-nav-tab">My Government</a>
					<div class="megadropdown-region" id="governmentItem">
						<div class="smleft">
								<div class="fourcolumns">
									<ul class="item">
										<li><h4><?php echo l("Mayor Mark Kruzan", "node/643"); ?>:</h4></li>
										<li><?php echo l("City News", "news"); ?></li>
										<li><a href="/proclamations">Proclamations</a></li>
										<li>More</li>
									</ul>
									<ul class="item">
										<li><h4><?php echo l("Common Council", "node/310"); ?>:</h4></li>
										<li>Meeting Schedule and Agenda</li>
										<li>Meeting Minutes</li>
										<li>Contacting the Council</li>
										<li>Find your Council Representative</li>
									</ul>
									<ul class="item">
										<li><h4>Topics:</h4></li>
										<li>Doing Business with the City</li>
										<li>Employment Opportunities</li>
										<li><a href="/code">Municipal Code</a></li>
										<li><?php echo l("See more about Government", "government"); ?></li>
									</ul>
									<ul class="item">
										<li><h4>See All:</h4></li>
										<li><a href="/boards">City Boards and Commissions</a></li>
										<li><a href="/departments">City Departments</a></li>
									</ul>
								</div>							
						</div>
						<div class="smright">
							<h4><a href="<?php echo $base_path; ?>contact">Contact the City</a></h4>
							<p>
								<a href="<?php echo $base_path; ?>contact"><img src="/media/m/2014/3/6/100/5318b4fc606c9.png" /></a>
							</p>
						</div>
					</div>
				</li>
				<li class="main-nav-item">
					<a href="/drupal/node/129" class="main-nav-tab withDD">I Want to</a>

					<div class="megadropdown-region">
						<div class="smleft">
							<div class="fourcolumns">
								<ul class="item">
									<li><h4>Apply:/h4></li>
									<li><a href="/jobs">City Jobs</a></li>
									<li><?php echo l("Board or Commission", "node/132"); ?></li>
									<li><?php echo l("See more", "node/129"); ?></li>
								</ul>
								<ul>
									<li><h4>Report:</h4></li>
									<li>Use <a href="/ureport">uReport</a> to report issues or comments</li>
								</ul>
								<ul class="item">
									<li><h4>Pay:</h4></li>
									<li><?php echo l("Parking Ticket", "node/29"); ?></li>
									<li><?php echo l("Utility Bill", "node/912"); ?></li>
										<li><?php echo l("See more", "node/129"); ?></li>
								</ul>
								<ul class="item">
									<li><h4>Sign Up:</h4></li>
									<li><a href="https://bloomington.in.gov/webtrac/cgi-bin/wspd_cgi.sh/wb1000.html?wbp=1">Parks &amp; Recreation Dept programs</a></li>
									<li>Utilities Services</li>
									<li><?php echo l("Pet behavior consultation", "node/29"); ?></li>
									<li><?php echo l("See more", "node/129"); ?></li>
								</ul>
								<ul class="item">
									<li><h4>Reserve:</h4></li>
									<li><a href="/node/716">Parks &amp; Recreation Dept Facilities</a></li>
									<li><a href="https://bloomington.in.gov/webtrac/cgi-bin/wspd_cgi.sh/wb1000.html?wbp=1">Park Shelterhouses</a></li>
									<li><a href="https://bloomington.in.gov/webtrac/cgi-bin/wspd_cgi.sh/wb1000.html?wbp=1">Tee Time</a></li>
									<li><?php echo l("See more", "node/129"); ?></li>
								</ul>
							</div>
						</div>
						<div class="smright">
							<h4><?php echo l("Utility Bill", "node/912"); ?></h4>
							<p>
								<a href="<?php echo $base_path; ?>node/912>"><img src="/media/m/2014/3/7/100/5319f60fda7c0.png" /></a>
							</p>
						</div>
					</div>
				</li>
				<li class="main-nav-item">
					<a href="/drupal/node/128" class="main-nav-tab withDD">Learn About</a>

					<div class="megadropdown-region">
						<div class="smleft">
							<div class="fivecolumns">
								<ul class="item">
									<li>About Bloomington</li>
									<li><?php echo l("Accessibility", "node/177"); ?></li>
									<li><?php echo l("Animals", "node/91"); ?></li>
									<li><?php echo l("Arts & Culture", "node/241"); ?></li>
									<li><?php echo l("Biking & Walking", "node/146"); ?></li>
									<li><?php echo l("Community Civility", "node/666"); ?></li>										
									<li><?php echo l("Community Events", "node/272"); ?></li>										
								</ul>
								<ul class="item">
									<li><?php echo l("Diversity", "node/151"); ?></li>										
									<li><?php echo l("Employment", "node/261"); ?></li>										
									<li><?php echo l("Farmers Market", "node/40"); ?></li>
									<li><?php echo l("Grants and Funding", "node/262"); ?></li>										
									<li><?php echo l("Health Resources", "node/94"); ?></li>										
									<li><?php echo l("Historic Preservation", "node/263"); ?></li>										
									<li><?php echo l("Housing", "node/93"); ?></li>
								</ul>
								<ul class="item">
									<li><?php echo l("Maps & GIS", "node/94"); ?></li>										
									<li><?php echo l("Neighborhoods", "node/265"); ?></li>
									<li><a href="/news">News</a></li>
									<li><?php echo l("Parking", "node/92"); ?></li>
									<li><?php echo l("Parks & Recreation Programs", "node/103"); ?></li>
								</ul>
								<ul class="item">
									<li><?php echo l("Public Safety", "node/266"); ?></li>
									<li><?php echo l("Rentals & Reservations", "node/107"); ?></li>
									<li><a href="/residential-services">Residential Services</a></li>
									<li><?php echo l("Roads and Traffic", "node/95"); ?></li>
									<li><?php echo l("Sports & Fitness", "node/102"); ?></li>
									<li><?php echo l("Sustainable City", "node/268"); ?></li>
									<li><?php echo l("Trash & Recycling", "node/940"); ?></li>
									<li><?php echo l("Volunteering", "node/270"); ?></li>
								</ul>
								<ul class="item">
									<li><h4>Find Places:</h4></li>
									<li><?php echo l("City Facilities", "node/98"); ?></li>
									<li><?php echo l("Community Facilities", "node/130"); ?></li>
									<li><?php echo l("Parks and Trails", "node/97"); ?></li>
									<li><?php echo l("Public Parking Garages and Lots", "node/111"); ?></li>
								</ul>
							</div>
					</div>
						<div class="smright">
							<h4><?php echo l("Health Resources", "node/94"); ?></h4>
							<p><a href="<?php echo $base_path; ?>/health-resources"><img src="/media/m/2014/3/6/100/5318eec208e5a.png" /></a></p>
						</div>
					</div>
				</li>
			<li class="main-nav-item">
				<a href="<?php echo $base_path; ?>node/5" class="main-nav-tab">Students</a>
			</li>
			<li class="main-nav-item">
				<a href="<?php echo $base_path; ?>node/951" class="main-nav-tab">Businesses</a>
			</li>
			<li class="main-nav-item">
				<a href="<?php echo $base_path; ?>node/7" class="main-nav-tab">Residents</a>
			</li>
			<li class="main-nav-item">
				<a href="<?php echo $base_path; ?>node/8" class="main-nav-tab">Visitors</a>
			</li>
			<li class="sm-links">
				<a href="https://twitter.com/citybloomington">
					<i class="icon-twitter"></i>
					<span class="hidden-label">Twitter</span>
				</a>
			</li>
			<li class="sm-links">
				<a href="http://youtube.com/user/cityofbloomington">
					<i class="icon-youtube"></i>
					<span class="hidden-label">Youtube</span>
				</a>
			</li>
			<li class="sm-links">
				<a href="http://facebook.com/cityofbloomington">
					<i class="icon-facebook"></i>
					<span class="hidden-label">Facebook</span>
				</a>
			<li>
		</ul>
